...
class _CheckoutScreenState extends ConsumerState<CheckoutScreen> {
      ...
      if (permission == LocationPermission.whileInUse || permission == LocationPermission.always) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('আপনার বর্তমান লোকেশন সরাসরি লক করা হয়েছে।')),
          );
        }
      }
    } catch (e) {
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('GPS এরর: $e')));
    } finally {
  ...
  Widget build(BuildContext context) {
    ...
    userAddressesAsync.whenData((addresses) {
      if (addresses.isEmpty && _useSavedAddress) {
        WidgetsBinding.instance.addPostFrameCallback((_) {
          if (mounted && _useSavedAddress) setState(() => _useSavedAddress = false);
        });
      }
    });
      ...
      body: userAsync.when(
        ...
        data: (user) {
          ...
          return areasAsync.when(
            ...
            error: (e, _) => Center(child: Text('Error: $e')),
            data: (areasList) {
              if (allAreas.isEmpty) return const Center(child: Text('No areas available'));
              ...
              final AreaModel orderArea = allAreas.firstWhere(
                ...
                orElse: () => allAreas.first, // Fallback to first area if not matched
              );
                ...
                data: (settings) {
                  return businessAsync.when(
                    ...
                    data: (business) {
                      ...
                      if (isSalon && _paymentMethod != 'upi') {
                        Future.delayed(Duration.zero, () {
                          if (mounted) setState(() => _paymentMethod = 'upi');
                        });
                      }
                            ...
                            child: Row(
                              children: [
                                ...
                                Column(
                                  ...
                                  children: [
                                    Text(isSalon ? 'Confirm Booking' : 'Checkout', style: GoogleFonts.urbanist(color: Colors.white, fontSize: 22, fontWeight: FontWeight.w800)),
                                    Text(isSalon ? 'Complete your appointment' : 'Complete your order', style: GoogleFonts.urbanist(color: Colors.white.withOpacity(0.8), fontSize: 13, fontWeight: FontWeight.w500)),
                                  ],
                                ),
                              ],
                            ),
                              ...
                              children: [
                                      ...
                                      child: Row(
                                        children: [
                                          ...
                                          Column(
                                            crossAxisAlignment: CrossAxisAlignment.start,
                                            children: [
                                              Text('Scheduled Slot', style: GoogleFonts.urbanist(fontSize: 11, color: Colors.grey, fontWeight: FontWeight.bold)),
                                              Text('${DateFormat('dd MMM').format(selectedDate)} at ${selectedSlot ?? "Not selected"}', style: GoogleFonts.urbanist(fontSize: 15, fontWeight: FontWeight.w800)),
                                            ],
                                          ),
                                        ],
                                      ),
                                ...
                                if (!isSalon)
                                  _SectionCard(
                                    ...
                                    child: userAddressesAsync.when(
                                      loading: () => const Center(child: Padding(padding: EdgeInsets.all(10), child: CircularProgressIndicator(strokeWidth: 2))),
                                        ...
                                        if (addresses.isEmpty) {
                                          return SizedBox(
                                            ...
                                            child: ElevatedButton.icon(
                                              ...
                                              label: Text('ADD DELIVERY ADDRESS', style: GoogleFonts.urbanist(fontWeight: FontWeight.w900, fontSize: 13, letterSpacing: 0.5)),
                                          ...
                                          children: [
                                            if (_selectedAddress != null)
                                              Container(
                                                ...
                                                decoration: BoxDecoration(color: AppColors.primary.withOpacity(0.05), borderRadius: BorderRadius.circular(15), border: Border.all(color: AppColors.primary.withOpacity(0.1))),
                                                child: Row(
                                                  children: [
                                                    ...
                                                    Expanded(
                                                      child: Column(
                                                        crossAxisAlignment: CrossAxisAlignment.start,
                                                        children: [
                                                          Text(_selectedAddress!.village, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
                                                          Text('${_selectedAddress!.houseNumber}, ${_selectedAddress!.landmark}', style: const TextStyle(fontSize: 12, color: Colors.grey)),
                                                        ],
                                                      ),
                                                    ),
                                                    TextButton(
                                            ...
                                            TextButton.icon(
                                              ...
                                              label: const Text('MANAGE ALL ADDRESSES', style: TextStyle(fontSize: 11, fontWeight: FontWeight.bold))
                                            ),
                                          ],
                                        );
                                      },
                                ...
                                if (!isSalon)
                                  _SectionCard(
                                    ...
                                    child: _premiumField(_noteCtrl, 'Delivery Note (Optional)', Icons.edit_note_rounded, maxLines: 2),
                                  ),
                                if (!isSalon) const SizedBox(height: 20),
                                  ...
                                  child: Column(
                                    children: [
                                      ...
                                      _PaymentTile(
                                        ...
                                        subtitle: isSalon ? 'Full prepaid booking required' : 'Safe & secure transactions',
                                ...
                                _SectionCard(
                                  ...
                                  child: Column(
                                    children: [
                                      ...
                                      _billRow('Delivery Charge', deliveryCharge, isFree: deliveryCharge == 0),
                                      ...
                                      Row(
                                        ...
                                        children: [
                                          Text('Total Amount', style: GoogleFonts.urbanist(fontSize: 18, fontWeight: FontWeight.w900, color: AppColors.charcoal)),
                                          Text('₹${total.toInt()}', style: GoogleFonts.urbanist(fontSize: 20, fontWeight: FontWeight.w900, color: AppColors.primary)),
                                        ],
                                      ),
                                    ],
                                  ),
                            ...
                            child: Row(
                              children: [
                                Column(
                                  ...
                                  crossAxisAlignment: CrossAxisAlignment.start,
                                  children: [
                                    Text('₹${total.toInt()}', style: GoogleFonts.urbanist(fontSize: 22, fontWeight: FontWeight.w900, color: AppColors.charcoal)),
                                    Text(isSalon ? 'BOOKING TOTAL' : 'GRAND TOTAL', style: GoogleFonts.urbanist(fontSize: 10, fontWeight: FontWeight.w800, color: AppColors.primary, letterSpacing: 0.5)),
                                  ],
                                ),
                                  ...
                                  child: Column(
                                    ...
                                    children: [
                                      if (belowMinimum)
                                        Padding(
                                          ...
                                          child: Text('Minimum order: ₹${orderArea.minimumOrder.toInt()}', 
                                            style: const TextStyle(color: Colors.red, fontSize: 10, fontWeight: FontWeight.bold)),
                                        ),
                                      ElevatedButton(
                                        onPressed: (belowMinimum || _isPlacing || _selectedAddress == null || _selectedAddress?.latitude == null)
                                            ? null
                                            : () async {
                                          // 1. VALIDATE ADDRESS/GPS
                                          if (_selectedAddress == null) {
                                            ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Please select a delivery address first')));
                                            return;
                                          }
                                          
                                          // 2. CHECK MINIMUM ORDER
                                          if (belowMinimum) {
                                            ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Minimum order amount not met (₹${orderArea.minimumOrder.toInt()})')));
                                            return;
                                          }
                                          
                                          // 3. VALIDATE GPS PIN
                                          if (_selectedAddress?.latitude == null) {
                                            ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('আপনার ঠিকানায় ম্যাপ পিন নেই। দয়া করে এড্রেসটি এডিট করে ম্যাপে পিন দিন।'), backgroundColor: Colors.orange));
                                            return;
                                          }

                                          // REDEFINE isSalon for validation
                                          final bool isSalonOrder = itemsList.any((item) => item.food.category.toLowerCase().contains('salon'));
                                          
                                          // ONLY REQUIRE SLOT FOR SALON
                                          if (isSalonOrder && selectedSlot == null) {
                                          ...
                                          if (isSalonOrder) {
                                            ...
                                            try {
                                              final checkTime = '${DateFormat('yyyy-MM-dd').format(selectedDate)} | $selectedSlot';
                                          ...
                                          if (existing != null) {
                                            if (mounted) {
                                              ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('SORRY! This slot was just booked by someone else. Please pick another time.'), backgroundColor: Colors.red));
                                            }
                                            return;
                                          }
                                        } catch (e) {
                                      ...
                                      if (_paymentMethod == 'upi') {
                                        ...
                                        await paymentService.openPayment(
                                          ...
                                          onSuccess: (paymentId) async {
                                            await _processOrderPlacement(ref, user, orderArea, itemsList, subtotal, total, platformFee, gst, 'paid', paymentId);
                                          },
                                          onFailure: (error) {
                                        ...
                                        await _processOrderPlacement(ref, user, orderArea, itemsList, subtotal, total, platformFee, gst, 'pending', null);
                                      }
                                    },
                                    ...
                                    child: _isPlacing
                                        ...
                                        : Row(
                                            ...
                                            children: [
                                              Text(isSalon ? 'CONFIRM BOOKING' : 'PLACE ORDER', style: GoogleFonts.urbanist(fontSize: 16, fontWeight: FontWeight.w900, color: Colors.white, letterSpacing: 1)),
      ...
      builder: (context) {
        ...
        return Container(
          ...
          child: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              Text('Select Address', style: GoogleFonts.urbanist(fontSize: 18, fontWeight: FontWeight.w800)),
              const SizedBox(height: 20),
              Flexible(
                child: ListView.builder(
                  ...
                  itemBuilder: (ctx, i) {
                    ...
                    return ListTile(
                      ...
                      title: Text(a.village, style: const TextStyle(fontWeight: FontWeight.bold)),
                      subtitle: Text('${a.houseNumber}, ${a.landmark}'),
                      onTap: () {
                        setState(() {
                          _selectedAddress = a;
                          _instantLat = a.latitude; _instantLon = a.longitude;
              ...
              TextButton.icon(
                onPressed: () {
                  ...
                  setState(() {
                    ...
                    _houseCtrl.clear(); _villageCtrl.clear(); _landmarkCtrl.clear(); _pinCtrl.clear();
                    _instantLat = null; _instantLon = null;
                  });
                },
  ...
  Future<void> _processOrderPlacement(WidgetRef ref, var user, AreaModel area, var items, double subtotal, double total, double platformFee, double gst, String paymentStatus, String? paymentId) async {
    setState(() => _isPlacing = true);
    final cartNotifier = ref.read(cartProvider.notifier);
    final restaurantId = items.first.food.restaurantId;
    
    // FETCH BUSINESS INFO FOR FINAL CHECK
    final business = await ref.read(businessRepositoryProvider).getBusinessesOnce(user.areaId!).then((list) => list.firstWhere((b) => b.id == restaurantId));
    ...
    try {
      ...
      if (mounted) {
        cartNotifier.clear();
        Navigator.pushReplacement(context, MaterialPageRoute(builder: (_) => OrderSuccessScreen(orderId: orderId)));
      }
    } catch (e) {
  ...
  Widget _billRow(String label, double value, {bool isFree = false}) {
    return Padding(
      ...
      child: Row(
        mainAxisAlignment: MainAxisAlignment.spaceBetween,
        children: [
          Text(label, style: GoogleFonts.urbanist(fontSize: 14, fontWeight: FontWeight.w500, color: AppColors.muted)),
          Text(
            isFree ? 'FREE' : '₹${value.toInt()}',
            style: GoogleFonts.urbanist(fontSize: 14, fontWeight: FontWeight.w700, color: isFree ? Colors.green : AppColors.charcoal),
          ),
        ],
      ),
    );
  ...
  Widget _premiumField(TextEditingController ctrl, String label, IconData icon, {bool isNumber = false, int maxLines = 1}) {
    return TextField(
      controller: ctrl,
      ...
      decoration: InputDecoration(
        labelText: label,
        labelStyle: GoogleFonts.urbanist(fontSize: 13, color: Colors.grey, fontWeight: FontWeight.w500),
        ...
        children: [
          Row(
            children: [
              ...
              const SizedBox(width: 8),
              Text(title, style: GoogleFonts.urbanist(fontSize: 15, fontWeight: FontWeight.w800, color: AppColors.charcoal)),
            ],
          ),
    ...
    return InkWell(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(color: selected ? AppColors.primary : Colors.grey.shade100, width: 1.5),
          borderRadius: BorderRadius.circular(20),
        ),
        child: Row(
          children: [
            Container(
              ...
              decoration: BoxDecoration(color: selected ? AppColors.primary.withOpacity(0.1) : Colors.grey.shade50, shape: BoxShape.circle),
            ...
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(label, style: GoogleFonts.urbanist(fontSize: 14, fontWeight: FontWeight.w800, color: AppColors.charcoal)),
                  Text(subtitle, style: GoogleFonts.urbanist(fontSize: 11, color: AppColors.muted, fontWeight: FontWeight.w500)),
                ],
              ),
            ),
            Icon(selected ? Icons.check_circle_rounded : Icons.radio_button_off_rounded, color: selected ? AppColors.primary : Colors.grey.shade300, size: 22),
          ],
        ),
      ),
    );
